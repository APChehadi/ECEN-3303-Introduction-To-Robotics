#!/usr/bin/python
# Copyright 2021 Prof. Bradley Hayes <bradley.hayes@colorado.edu>
...
def get_nd_obstacle(state_bounds):
    ...
    radius = random.random() * 0.6 + 0.1 # n-dim hypersphere of radius [0.1,0.7]
    return [np.array(center_vector), radius]

def setup_random_2d_world():
    return setup_random_world(2)

def setup_random_world(n_dims=2, min_range=0, max_range=10, n_obs=30):
    '''
    ...
    '''
    state_bounds = np.array([[min_range,max_range] for i in range(n_dims)]) # matrix of min/max values for each dimension
    obstacles = [] # [pt, radius] spherical obstacles
...
def get_nearest_vertex(node_list, q_point):
    ...
    for i in range(node_list_length):
        temp_node = np.array(node_list[i].point)
        temp_distance = np.linalg.norm(temp_node - np.array(q_point))
    ...
    return node_list[nearest_vertex_index]
...
def steer(from_point, to_point, delta_q):
    ...
    :returns path: Array of points leading from "from_point" to "to_point" (inclusive of endpoints)  (e.g., [ [1.,2.], [1., 1.], [0., 0.] ])
    '''

    original_from_point = np.array(from_point)
    original_to_point = np.array(to_point)
    original_vector_length = np.linalg.norm(original_to_point - original_from_point)

    if original_vector_length > delta_q:
        path_from_point = original_from_point
        delta_q_vector = ((original_to_point - original_from_point) / original_vector_length) * delta_q
        path_to_point = np.array(path_from_point + delta_q_vector)
    else:
        path_from_point = original_from_point
        path_to_point = original_to_point
    
    path = np.linspace(path_from_point, path_to_point, num=10)
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, k, delta_q):
    ...
    If goal_point is None, it should build a graph without a goal nearest_vertex_index and terminate after k iterations.
    ...
    goal_point_is_none = np.array(goal_point).all() == None

    for k_index in range(k):
        path_is_valid = False

        while path_is_valid == False:
            temp_q_point = get_random_valid_vertex(state_is_valid, state_bounds)

            if (goal_point_is_none == False):
                if (random.random() < 0.05):
                    temp_q_point = goal_point

            temp_closest_node = get_nearest_vertex(node_list, temp_q_point)

            temp_path = steer(temp_closest_node.point, temp_q_point, delta_q)

            if check_path_valid(temp_path, state_is_valid):
                new_node = Node(temp_path[-1], temp_closest_node)
                new_node.path_from_parent = temp_path
                node_list.append(new_node)
                path_is_valid = True

                if (goal_point_is_none == False):
                    goal_distance = np.linalg.norm(np.array(temp_q_point) - np.array(goal_point))
